...
function cmdPaperInit(cwd, topic, raw) {
  ...
  const docsScriptPath = path.join(installRoot, 'scripts', 'gen-docs.js').replace(/\\/g, '/');
  const docsBuildCommand = `node "${docsScriptPath}" paper .tmp/paper.md`;
  const docsPdfCommand = `node "${docsScriptPath}" paper .tmp/paper.md --pdf`;
  ...
  for (const section of sectionFiles) {
    writeIfMissing(
      ...
      `# ${section.title}\n\n## Objective\n\n${section.prompt}\n\n## Manuscript Draft\n\n_TODO: Write publication-ready narrative prose, not checklist bullets. For standard sections, target 4-8 paragraphs with clear transitions and argument flow. Keep bullets minimal (0-2 short bullets if needed)._\n\n_TODO: Use inline citations for factual claims, for example [SRC-01], [SRC-02]._\n\n_TODO: Place tables, diagrams, and image embeds directly in this section where evidence is discussed._\n\n## Citation Coverage\n\n| Claim Cluster | Source IDs | Counterevidence Source IDs | Confidence |\n|---------------|------------|----------------------------|------------|\n| _TODO_ | [SRC-##] | [SRC-## or none] | _TODO_ |\n\n## Reviewer Notes (Not for Publication)\n\nUse only for author-side caveats that should not appear in the final manuscript.\n\n- Main uncertainty:\n- Methodological caveat:\n- Follow-up evidence to collect:\n`
    );
  }

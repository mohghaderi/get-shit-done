...
<required_reading>
@~/.claude/get-paper-done/references/verification-patterns.md
@~/.claude/get-paper-done/templates/verification-report.md
</required_reading>
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs verify key-links "$plan")
...
See ~/.claude/get-paper-done/templates/verification-report.md for complete template.
</step>

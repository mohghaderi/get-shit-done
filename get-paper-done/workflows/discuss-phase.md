...
1. **papergen-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **papergen-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
```
Phase: "User citation verification"
→ Session handling, Error responses, Multi-device policy, Recovery flow

Phase: "Organize photo library"
→ Grouping criteria, Duplicate handling, Naming convention, Folder structure

Phase: "CLI for source ledger backups"
→ Output format, Flag design, Progress reporting, Error recovery

Phase: "evidence interface documentation"
...
```bash
INIT=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs init phase-op "${PHASE}")
```
...
Use /papergen:progress to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /papergen:plan-phase {X} to replan
...
For "source ledger backup CLI" (command-line tool):
```
...
**Also available:**
- `/papergen:plan-phase ${PHASE} --skip-research` — plan without research
...
```bash
node ~/.claude/get-paper-done/bin/papergen-tools.cjs commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
node ~/.claude/get-paper-done/bin/papergen-tools.cjs commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
Task(
  prompt="
    ...
    <execution_context>
    @~/.claude/get-paper-done/workflows/plan-phase.md
    ...
    3. When spawning agents (papergen-phase-researcher, papergen-planner, papergen-plan-checker), use Task with specified subagent_type and model
    ...
    5. Do NOT use the Skill tool or /papergen: commands. Read workflow .md files directly.
...
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ...
  Continue: /papergen:execute-phase ${PHASE}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /papergen:plan-phase ${PHASE}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /papergen:plan-phase ${PHASE} --gaps
  ```

<purpose>
Execute small, ad-hoc tasks with GPD guarantees (atomic commits, STATE.md tracking). Quick mode spawns papergen-planner (quick mode) + papergen-executor(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
```bash
INIT=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs init quick "$DESCRIPTION")
```
...
**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/papergen:new-project` first.
...
Spawn papergen-executor with plan reference:
...
```bash
node ~/.claude/get-paper-done/bin/papergen-tools.cjs commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
...
GPD > QUICK TASK COMPLETE (FULL MODE)
...
Ready for next task: /papergen:quick
```

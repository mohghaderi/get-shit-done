...
if [ -f ./.claude/get-paper-done/VERSION ]; then
  cat ./.claude/get-paper-done/VERSION
  echo "LOCAL"
elif [ -f ~/.claude/get-paper-done/VERSION ]; then
  cat ~/.claude/get-paper-done/VERSION
  echo "GLOBAL"
else
  echo "UNKNOWN"
...
```bash
npm view get-paper-done-cc version 2>/dev/null
```
...
To update manually: `npx get-paper-done-cc --global`
```
...
⚠️  **Note:** The installer performs a clean install of GPD folders:
- `commands/papergen/` will be wiped and replaced
- `get-paper-done/` will be wiped and replaced
...
- Custom agents not prefixed with `gpd-` ✓
- Custom hooks ✓
- Your CLAUDE.md files ✓

If you've modified any GPD files directly, they'll be automatically backed up to `gpd-local-patches/` and can be reapplied with `/papergen:reapply-patches` after the update.
```
...
**If LOCAL install:**
```bash
npx get-paper-done-cc --local
```

**If GLOBAL install (or unknown):**
```bash
npx get-paper-done-cc --global
```
...
[View full changelog](https://github.com/mohghaderi/get-paper-done/blob/main/CHANGELOG.md)
```
</step>

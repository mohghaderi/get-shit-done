...
```bash
INIT=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs init milestone-op)
```
...
```bash
CHECKER_MODEL=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs resolve-model papergen-integration-checker --raw)
```
...
node ~/.claude/get-paper-done/bin/papergen-tools.cjs phases list
```
...
PHASE_INFO=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node ~/.claude/get-paper-done/bin/papergen-tools.cjs summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```

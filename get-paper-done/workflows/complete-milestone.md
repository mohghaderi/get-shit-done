...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node ~/.claude/get-paper-done/bin/papergen-tools.cjs summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
```bash
ARCHIVE=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
```bash
INIT=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs init execute-phase "1")
```
...
```bash
node ~/.claude/get-paper-done/bin/papergen-tools.cjs commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```

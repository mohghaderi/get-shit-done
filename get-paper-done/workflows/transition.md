...
```bash
TRANSITION=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs progress bar --raw)
```
...
```bash
node ~/.claude/get-paper-done/bin/papergen-tools.cjs config-set workflow.auto_advance false
```

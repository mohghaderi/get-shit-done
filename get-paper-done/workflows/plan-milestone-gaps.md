...
PHASES=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs phases list)
...
```bash
node ~/.claude/get-paper-done/bin/papergen-tools.cjs commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```

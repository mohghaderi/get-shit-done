...
```bash
PHASE_INFO=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/get-paper-done/bin/papergen-tools.cjs find-phase "${PHASE}" --raw)
```

...
```bash
INIT=$(node ~/.claude/get-shit-done/bin/papergen-tools.cjs init new-paper)
```
...
```bash
node ~/.claude/get-shit-done/bin/papergen-tools.cjs paper init "$ARGUMENTS"
```
...
4. Write section draft text with explicit in-text source IDs like `[SRC-03]`.
5. Add an "Evidence Notes" block listing claims and supporting source IDs.
6. For each major finding, record at least one counterevidence source or explicitly note none found.
...
- Objective
- Draft prose
- Evidence notes with source IDs
...
```bash
node ~/.claude/get-shit-done/bin/papergen-tools.cjs commit "docs: initialize research paper workspace" --files paper/
```

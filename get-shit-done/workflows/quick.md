<purpose>
Execute small, ad-hoc tasks with GSD guarantees (atomic commits, STATE.md tracking). Quick mode spawns papergen-planner (quick mode) + papergen-executor(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
```bash
INIT=$(node ~/.claude/get-shit-done/bin/papergen-tools.cjs init quick "$DESCRIPTION")
```
...
```bash
node ~/.claude/get-shit-done/bin/papergen-tools.cjs commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```

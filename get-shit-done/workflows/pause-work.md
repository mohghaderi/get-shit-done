...
```bash
timestamp=$(node ~/.claude/get-shit-done/bin/papergen-tools.cjs current-timestamp full --raw)
```
</step>

<step name="commit">
```bash
node ~/.claude/get-shit-done/bin/papergen-tools.cjs commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md
```
</step>

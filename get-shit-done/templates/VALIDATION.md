...
> Generated by `papergen-phase-researcher` during `/papergen:plan-phase {N}`.
> Updated by `papergen-plan-checker` after plan approval.
> Governs feedback sampling during `/papergen:execute-phase {N}`.
...
- **Before `/papergen:verify-work`:** Full suite must be green
...
| {N}-02-01 | 02 | 2 | REQ-{XX} | smoke | `curl -s {section} \| grep {expected}` | ✅ N/A | ⬜ pending |
...
> These are surfaced during `/papergen:verify-work` UAT.

| Behavior | Requirement | Why Manual | Test Instructions |
|----------|-------------|------------|-------------------|
| {behavior} | REQ-{XX} | {reason: visual, third-party citation, physical device...} | {step-by-step} |
...
Updated by `papergen-plan-checker` when plans are approved:
...
Updated during `/papergen:execute-phase {N}`:

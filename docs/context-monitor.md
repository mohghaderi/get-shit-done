...
| CRITICAL | <= 25% | Stop immediately, save state (`/papergen:pause-work`) |
...
```
Statusline Hook (gpd-statusline.js)
    | writes
    v
/tmp/claude-ctx-{session_id}.json
    ^ reads
    |
Context Monitor (gpd-context-monitor.js, PostToolUse)
    | injects
    v
...
GPD's `/papergen:pause-work` command saves execution state. The WARNING message suggests using it. The CRITICAL message instructs immediate state save.

## Setup

Both hooks are automatically registered during `npx get-paper-done-cc` installation:
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node ~/.claude/hooks/papergen-statusline.js"
  },
  "hooks": {
    "PostToolUse": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/hooks/papergen-context-monitor.js"
          }
        ]
      }
    ]
